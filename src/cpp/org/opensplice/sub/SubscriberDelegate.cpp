  ...
  mask_(event_mask)
{
	...
	dp->dp_->get_default_subscriber_qos(sqos);
        ...
        DDS::Subscriber* s =
			dp->dp_->create_subscriber(sqos, 0, DDS::ANY_STATUS);

	if (s == 0)
		throw dds::core::PreconditionNotMetError("Exception, Unable create Subscriber!");
...
void SubscriberDelegate::qos(const dds::sub::qos::SubscriberQos& the_qos) {
